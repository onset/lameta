...
function process(allEntries) {
  ...
  console.log(
    "Now check with `pnpm exec vitest run src/languageFinder/langindex.spec.ts` and then all other tests with `pnpm run test -- --run` and `pnpm run e2e`."
  );
