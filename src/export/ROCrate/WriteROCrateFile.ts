  ...
  if (await fs.pathExists(roCrateFilePath)) {
    try {
      ...
      if (!(stats.mode & 0o200)) {
        ...
        throw new Error(
          `Cannot overwrite read-only file: ${roCrateFilePath}. Please make the file writable or delete it manually.`
        );
      }
      ...
      if (error.code === "EACCES" || error.code === "EPERM") {
        throw new Error(
          `Permission denied: Cannot overwrite ${roCrateFilePath}. The file may be read-only or in use by another program.`
        );
      }
