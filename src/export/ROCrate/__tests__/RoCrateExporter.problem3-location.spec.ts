...
describe("RoCrateExporter Problem 3 - Location Fields", () => {
  it("should combine locationRegion, locationCountry, and locationContinent into Place description", async () => {
    ...
    const placeEntity = roCrate["@graph"].find(
      (entity: any) => entity["@id"] === locationId
    );
    ...
    if (placeEntity.description) {
      ...
      console.log(
        "✅ Place entity has combined location description:",
        placeEntity.description
      );
    } else {
      console.log(
        "⚠️ Place entity doesn't have a description (no companion location fields found)"
      );
    }
    ...
    console.log(
      "✅ No session entities have locationRegion, locationCountry, or locationContinent properties"
    );
  });
});
