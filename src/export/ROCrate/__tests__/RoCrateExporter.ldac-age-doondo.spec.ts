...
describe.skipIf(!process.env.ENABLE_EXPENSIVE_TESTS)(
  ...
  () => {
    it("should have ldac:age as integer values, not strings (actual Doondo project)", async () => {
      ...
      const personEntitiesWithAge = roCrateJson["@graph"].filter((entity: any) => 
        entity["@type"]?.includes("Person") &&
        entity["ldac:age"] !== undefined
      );

      console.log(`Found ${personEntitiesWithAge.length} person entities with ldac:age property`);

      if (personEntitiesWithAge.length > 0) {
        console.log("Sample person with ldac:age:", JSON.stringify(personEntitiesWithAge[0], null, 2));
        ...
        personEntitiesWithAge.forEach((personEntity: any) => {
          ...
          if (typeof age === "string") {
            problemAges.push(`${personEntity["@id"]}: ldac:age is string "${age}" instead of number`);
          } else if (typeof age !== "number") {
            problemAges.push(`${personEntity["@id"]}: ldac:age is ${typeof age} instead of number`);
          }
        });
        ...
        if (problemAges.length > 0) {
          ...
          expect.fail(`Found ${problemAges.length} persons with non-integer ldac:age values:\n${problemAges.join('\n')}`);
        } else {
