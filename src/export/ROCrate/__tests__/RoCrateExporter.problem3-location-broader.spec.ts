...
describe("RoCrateExporter Problem 3 - Location Fields Broader Test", () => {
  it("should check all sessions and places for location field handling", async () => {
    ...
    const entitiesWithBadLocationProps = roCrate["@graph"].filter((entity: any) => 
      entity.locationRegion || entity.locationCountry || entity.locationContinent
    );

    console.log(`Entities with problematic location properties: ${entitiesWithBadLocationProps.length}`);
    if (entitiesWithBadLocationProps.length > 0) {
      console.log("Examples:", entitiesWithBadLocationProps.slice(0, 3).map(e => ({
        id: e["@id"],
        type: e["@type"],
    ...
    const placeEntities = roCrate["@graph"].filter((entity: any) => entity["@type"] === "Place");
    console.log(`Total Place entities: ${placeEntities.length}`);

    // Count Place entities with descriptions
    const placeEntitiesWithDescriptions = placeEntities.filter((entity: any) => entity.description);
    console.log(`Place entities with descriptions: ${placeEntitiesWithDescriptions.length}`);
    ...
    const sessionEntities = roCrate["@graph"].filter((entity: any) => entity["@id"]?.includes("Sessions/"));
    console.log(`Total Session entities: ${sessionEntities.length}`);

    // Count session entities with location references
    const sessionEntitiesWithLocation = sessionEntities.filter((entity: any) => entity.location);
    console.log(`Session entities with location references: ${sessionEntitiesWithLocation.length}`);
    ...
    console.log("✅ No entities have locationRegion, locationCountry, or locationContinent properties");
  });
});
