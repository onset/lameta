...
describe("RoCrateExporter location properties Doondo validation", () => {
  it("should not use non-standard location properties like locationRegion, locationCountry, locationContinent", async () => {
    ...
    const entitiesWithBadLocationProps = roCrateJson["@graph"].filter(
      (entity: any) =>
    ...
    console.log(
      `Found ${entitiesWithBadLocationProps.length} entities with non-standard location properties`
    );

    if (entitiesWithBadLocationProps.length > 0) {
      console.log(
        "Sample entity with problematic location properties:",
        JSON.stringify(entitiesWithBadLocationProps[0], null, 2)
      );

      const problemProps: string[] = [];
      entitiesWithBadLocationProps.forEach((entity: any) => {
        if (entity.locationRegion)
          problemProps.push(
            `${entity["@id"]}: has locationRegion "${entity.locationRegion}"`
          );
        if (entity.locationCountry)
          problemProps.push(
            `${entity["@id"]}: has locationCountry "${entity.locationCountry}"`
          );
        if (entity.locationContinent)
          problemProps.push(
            `${entity["@id"]}: has locationContinent "${entity.locationContinent}"`
          );
      });
      ...
      expect.fail(
        `Found ${entitiesWithBadLocationProps.length} entities with non-standard location properties. These should use 'location' property linking to Place entities instead.`
      );
    } else {
      console.log(
        "✅ No entities found using non-standard location properties"
      );
    }
