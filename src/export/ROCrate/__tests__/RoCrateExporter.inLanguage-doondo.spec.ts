...
describe.skipIf(!process.env.ENABLE_EXPENSIVE_TESTS)(
  ...
  () => {
    it("should have inLanguage as string values, not object references (actual Doondo project)", async () => {
      ...
      const sessionEntitiesWithInLanguage = roCrateJson["@graph"].filter((entity: any) => 
        entity["@id"]?.startsWith("Sessions/") && 
        entity["@type"]?.includes("Event") &&
        entity.inLanguage !== undefined
      );

      console.log(`Found ${sessionEntitiesWithInLanguage.length} session entities with inLanguage property`);

      if (sessionEntitiesWithInLanguage.length > 0) {
        console.log("Sample session with inLanguage:", JSON.stringify(sessionEntitiesWithInLanguage[0], null, 2));
        
        // Check that inLanguage values are strings, not object references
        sessionEntitiesWithInLanguage.forEach((sessionEntity: any, index: number) => {
          expect(typeof sessionEntity.inLanguage, 
            `Session ${sessionEntity["@id"]} inLanguage should be string, got: ${JSON.stringify(sessionEntity.inLanguage)}`
          ).toBe("string");
          
          expect(sessionEntity.inLanguage, 
            `Session ${sessionEntity["@id"]} inLanguage should not be an object with @id`
          ).not.toHaveProperty("@id");
        });
      } else {
        console.log("No session entities found with inLanguage property - this might be expected if using ldac:subjectLanguage instead");
        
        // Check if sessions have ldac:subjectLanguage instead
        const sessionEntitiesWithSubjectLang = roCrateJson["@graph"].filter((entity: any) => 
          entity["@id"]?.startsWith("Sessions/") && 
          entity["@type"]?.includes("Event") &&
          entity["ldac:subjectLanguage"] !== undefined
        );
        
        console.log(`Found ${sessionEntitiesWithSubjectLang.length} session entities with ldac:subjectLanguage property`);
        if (sessionEntitiesWithSubjectLang.length > 0) {
          console.log("Sample session with ldac:subjectLanguage:", JSON.stringify(sessionEntitiesWithSubjectLang[0], null, 2));
        }
      }
