...
describe("getLdacMaterialType", () => {
  it("should return ldac:PrimaryMaterial for audio files", () => {
    expect(getLdacMaterialType("Audio")).toBe(ldac("ldac:PrimaryMaterial"));
  });

  it("should return ldac:PrimaryMaterial for video files", () => {
    expect(getLdacMaterialType("Video")).toBe(ldac("ldac:PrimaryMaterial"));
  });

  it("should return ldac:PrimaryMaterial for image files", () => {
    expect(getLdacMaterialType("Image")).toBe(ldac("ldac:PrimaryMaterial"));
  });

  it("should return ldac:Annotation for ELAN files", () => {
    expect(getLdacMaterialType("ELAN")).toBe(ldac("ldac:Annotation"));
  });

  it("should return ldac:Annotation for CHAT files", () => {
    expect(getLdacMaterialType("CHAT")).toBe(ldac("ldac:Annotation"));
  });

  it("should return ldac:Annotation for document files", () => {
    expect(getLdacMaterialType("Doc")).toBe(ldac("ldac:Annotation"));
  });

  it("should return ldac:Annotation for Toolbox files", () => {
    expect(getLdacMaterialType("Toolbox")).toBe(ldac("ldac:Annotation"));
  });

  it("should return ldac:Annotation for Praat files", () => {
    expect(getLdacMaterialType("Praat")).toBe(ldac("ldac:Annotation"));
  });

  it("should return ldac:Annotation for FLEx files", () => {
    expect(getLdacMaterialType("FLEx")).toBe(ldac("ldac:Annotation"));
  });

  it("should return ldac:Annotation for unknown file types", () => {
    expect(getLdacMaterialType("Unknown")).toBe(ldac("ldac:Annotation"));
  });

  it("should return ldac:Annotation for empty string", () => {
    expect(getLdacMaterialType("")).toBe(ldac("ldac:Annotation"));
  });

  it("should return ldac:Annotation for undefined input", () => {
    expect(getLdacMaterialType(undefined as any)).toBe(ldac("ldac:Annotation"));
  });
});
...
describe("getLdacMaterialTypeForPath", () => {
  ...
  it("should return ldac:Annotation for annotation file paths", () => {
    expect(getLdacMaterialTypeForPath("transcription.eaf")).toBe(
      ldac("ldac:Annotation")
    );
    expect(getLdacMaterialTypeForPath("analysis.cha")).toBe(
      ldac("ldac:Annotation")
    );
    expect(getLdacMaterialTypeForPath("document.pdf")).toBe(
      ldac("ldac:Annotation")
    );
  });

  it("should return ldac:Annotation for unknown file extensions", () => {
    expect(getLdacMaterialTypeForPath("unknown.xyz")).toBe(
      ldac("ldac:Annotation")
    );
  });
});
